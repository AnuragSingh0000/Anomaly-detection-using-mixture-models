...
class ComputeLoss:
    def __init__(self, model, lambda_energy, lambda_cov, device, n_gmm):
        ...
        self.device = device
        self.n_gmm = n_gmm
        self.mu = 0.08134789422535264;
        self.std = 0.27097452611032846;
    
    def forward(self, x, x_hat, z, gamma, labels):
        """Computing the loss function for DAGMM."""
        reconst_loss = torch.mean((x - x_hat).pow(2))
        # print(z.shape, labels.shape, gamma.shape)
        sample_energy, cov_diag = self.compute_energy(z[labels == 0], gamma[labels == 0])
    ...
    def compute_prob(self, z, gamma, phi=None, mu=None, cov=None, sample_mean=True):
        ...
        E_z = -0.5 * torch.sum(torch.sum(z_mu.unsqueeze(-1) * cov_inverse.unsqueeze(0), dim=-2) * z_mu, dim=-1)
        E_z = torch.exp(E_z)
        E_z = (torch.sum(phi.unsqueeze(0) * E_z / (torch.sqrt(det_cov)).unsqueeze(0), dim=1) + eps)
        if sample_mean==True:
            E_z = torch.mean(E_z) 
        return E_z
