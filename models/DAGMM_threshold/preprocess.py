...
class KDDCupData:
    def __init__(self, data_dir, mode):
        ...
        # use the full dataset for trainin and testing.
        # however for the training data, only 50% of the normal data is labeled as 1 and others are labeled as 0
        
        if mode == "train":
            self.x = features
            self.y = labels
            
            self.y[labels == 0][:n_train] = 1
        elif mode == "test":
            self.x = features[n_train:]
            self.y = labels[n_train:]
